...
- [ ] **Step 2:** Escrever `test/retrieval.test.ts` com testes falhando: uma query que casa só `auth-guard.md` **e nenhum termo de `cache-wrapper.md`** ainda assim traz chunk de `cache-wrapper.md` marcado `viaGraph: true`, com score exatamente `0.4 ×` o do chunk de origem — é o teste que prova que a expansão alcança vocabulário que o BM25 não alcança. **Passar `limit: 12` explicitamente**: os 4 vizinhos de `auth-guard.md` contribuem 15 chunks, todos empatados no score herdado, e com o `DEFAULT_LIMIT` de 6 o orçamento se esgota antes de alcançar `cache-wrapper.md`. Um segundo teste roda a mesma query duas vezes e exige resultado idêntico, fixando o desempate; expansão é de um salto só (uma nota a dois saltos do resultado direto nunca aparece); dedupe mantém o maior score quando um chunk chega pelas duas vias; `01-raw/inbox/rascunho.md` não aparece na busca por `rascunhoexclusivo` sem `include_raw`, e aparece com ele; filtro `tipo: 'projeto'` devolve só `03-projects/potentia/README.md`; filtro `folder: '02-wiki/docker'` restringe corretamente; query sem match devolve `results: []` e `suggestions` não-vazio contendo `bullmq` para a query `bulmq`.
