...
function normalize(data: Record<string, unknown>): Frontmatter {
  ...
  for (const key of ['criado', 'atualizado'] as const) {
    ...
    if (value instanceof Date) fm[key] = toIsoDay(value);
  }
  return fm;
}

/** Local-time YYYY-MM-DD. `toISOString` would shift the day for negative UTC offsets. */
...
- [ ] **Step 2:** Escrever `test/retrieval.test.ts` com testes falhando: uma query que casa só `auth-guard.md` **e nenhum termo de `cache-wrapper.md`** ainda assim traz chunk de `cache-wrapper.md` marcado `viaGraph: true`, com score exatamente `0.4 ×` o do chunk de origem — este é o teste que prova que a expansão alcança vocabulário que o BM25 não alcança; expansão é de um salto só (uma nota a dois saltos do resultado direto nunca aparece); dedupe mantém o maior score quando um chunk chega pelas duas vias; `01-raw/inbox/rascunho.md` não aparece na busca por `rascunhoexclusivo` sem `include_raw`, e aparece com ele; filtro `tipo: 'projeto'` devolve só `03-projects/potentia/README.md`; filtro `folder: '02-wiki/docker'` restringe corretamente; query sem match devolve `results: []` e `suggestions` não-vazio contendo `bullmq` para a query `bulmq`.
...
- [ ] **Step 1:** Escrever `test/tools.test.ts` com testes falhando, chamando os handlers diretamente sem subir transporte: schema de `vault_search` rejeita `query` vazia; `vault_search` devolve texto contendo `caminho:linha` para cada resultado; `vault_get_note` de caminho inexistente devolve erro de tool legível, não exceção não tratada; `vault_get_note` de `auth-guard.md` lista o link quebrado; `vault_list` com `tipo: 'projeto'` devolve só `03-projects/potentia/README.md` — `_templates/projeto.md` também declara `tipo: projeto`, e é a exclusão de `_templates/` no scanner (Task 6) que o mantém fora; `vault_backlinks` de `auth-guard.md` traz **três** notas (`nestjs-moc.md`, `cache-wrapper.md` e `potentia/README.md` — a entrada do MOC é um backlink como qualquer outro); `vault_learn` com `dominio` inválido devolve erro citando os domínios válidos; busca sem resultado devolve texto de "sem match" com as sugestões.
