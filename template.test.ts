 ...
 * version put it in `<repo>/node_modules` so a bare `gray-matter` import would
 * resolve by directory position — but `teammates.gate.json` SYMLINKS
 * `node_modules` into the gate's preview tree, so that path is the one real
 * shared directory, not a private copy, and the test was writing into it. The
 * probe now imports the module by absolute specifier, which resolves from
 * anywhere on disk and needs no copy of anything.
 */
function runInTimezone(tz: string, snippet: string): string {
  const dir = mkdtempSync(join(tmpdir(), 'vault-mcp-tz-'));
  try {
    // Node runs the TypeScript source directly (type stripping), so the probe imports the real
    // module by absolute path instead of inlining a hand-transpiled copy of it. The previous
    // version called `ts.transpileModule`, whose JS API surface changed in TypeScript 7 — and
    // inlining meant the bare `gray-matter` import had to be rewritten by regex to resolve from
    // outside the repo. Importing by absolute specifier removes both problems.
    const entry = join(dir, 'probe.ts');
    writeFileSync(
      entry,
      `import { applyTemplate, ensureFrontmatter, formatLocal, TemplateError } from ${JSON.stringify(
        pathToFileURL(MODULE_SRC).href,
      )};\n${snippet}\n`,
    );
